...
class doubleSpace : public fileFilterDerivedEncryption
{
public:
	doubleSpace() {};
	void doubleSpaceMethod(ifstream &in, ofstream &out, char c)
	{
		in.clear();
		in.seekg(0, ios::beg); //set reader to beginning of input file
		while (!in.eof())
...
int main()
{
	...
	output.open("doubleSpaceText.txt");
	doubleSpace * m = new doubleSpace;
	m->doubleSpaceMethod(input, output, c);
	//g->doFilter(input, output);
	output.close();
